...
def generate_random_placement(benchmark, seed=42):
    ...
    # Keep fixed macros at their benchmark locations.
...
def write_openroad_placement_tcl(placement, benchmark, plc, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("# Written from partcl-macro-place-challenge\n")
        f.write("#\n")
        ...
        for i, macro_idx in enumerate(benchmark.hard_macro_indices):
            ...
            # place_macro expects the lower-left corner, not the center.
            x_ll = x - w / 2
            y_ll = y - h / 2

            # Reuse the instance orientation when it is set.
...
def _plc_extract_group_and_index(plc_name):
    ...
    # Convert the prefix to the ODB naming style.
...
def _plc_to_odb_name(plc_name):
    ...
    """
    # Split on the hierarchy separator first.
    parts = plc_name.split('/')
    result = []
    for part in parts:
        # Fold generate indices written as [N].
        part = re.sub(r'\[(\d+)\]\.', r'_\1__', part)
        # Handle a trailing [N].
        part = re.sub(r'\[(\d+)\]$', r'_\1_', part)
        # Remaining dots inside the segment become underscores.
        part = part.replace('.', '_')
        result.append(part)
    return '/'.join(result)
...
def write_orfs_macro_placement(placement, benchmark, plc, output_file, core_area=None, use_genus_names=False):
    ...
    # Group placements by (prefix, flat index).
    ...
    for i, macro_idx in enumerate(benchmark.hard_macro_indices):
        ...
        if use_genus_names:
            ...
            if group_prefix is not None:
                ...
                # Fall back to direct name conversion for other designs.
                odb_name = _plc_to_odb_name(plc_name)
                direct_placements.append((odb_name, x_ll, y_ll, orientation, plc_name))
                total_macros += 1

    # Add a minimum gap so the PDN channels have room.
    ...
    if core_area is not None:
        ...
        # Save the original coordinates for the spacing report.
            ...
            for i in range(len(_all)):
                for j in range(i + 1, len(_all)):
                    ...
                    if ox > -MIN_GAP and oy > -MIN_GAP and (ox > 0 or oy > 0):
                        # Push the pair apart along the easier axis.
    ...
    with open(output_file, 'w') as f:
        ...
        # TCL-side matching logic.
        f.write("""# Gather block instances by SRAM prefix.
...
# Sort each group by (genblk_idx, mem_idx), then walk the flat plc order.
set _placed 0
